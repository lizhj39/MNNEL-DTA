...
class DTADataset(Dataset):
    def __init__(self, root, path, target_graph, target_len):

        super(DTADataset, self).__init__(root)
        self.path = path
        df = pd.read_csv(path)
        self.data = []
        self.process(df, target_graph, target_len)
    ...
    def processed_file_names(self):
        return ['process_protein_structure.pt']
    ...
    def process(self, df, target_graph, target_len):
        # data_list = []
