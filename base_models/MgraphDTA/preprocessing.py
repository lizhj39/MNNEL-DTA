...
class GNNDataset(InMemoryDataset):
    ...
    def process(self):
        if os.path.exists(self.processed_paths[0]):
            ...
            print("Mgraph processced data already exists.")
        else:
            ...
            # empty_keys = [k for k, v in graph_dict.items() if not v]
            all_list = self.process_data(self.raw_paths[0], graph_dict)

            if self.pre_filter is not None:
                all_list = [train for train in all_list if self.pre_filter(train)]

            if self.pre_transform is not None:
                train_list = [self.pre_transform(all_) for all_ in all_list]
...
#     GNNDataset('../../data_input/davis')
#     GNNDataset('../../data_input/kiba')
