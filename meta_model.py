...
class TransformerRegressor(nn.Module):
    def __init__(self, save_path, drug_vocab, target_vocab, dataset_name: str, auto_dataset: bool=True, d_model=256, nhead=8, num_layers=1, seed=42, test_ratio=0.15):
        ...
        self.model_Mgraph.load_state_dict(torch.load(f"saved_models/Mgraph_{dataset_name}.pt"))
        ...
        self.model_TFusion.load_state_dict(torch.load(f"saved_models/TFusion_{dataset_name}.pt"))
        ...
        self.model_NHGNN = NHGNN_DTA(pretrain_path=f"saved_models/TFusion_{dataset_name}.pt",
                                     ...
                                     drug_vocab=drug_vocab, target_vocab=target_vocab, seed=seed, test_ratio=test_ratio)
        self.model_NHGNN.load_state_dict(torch.load(f"saved_models/NHGNN_{dataset_name}.pkl")['model'])
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=self.d_model, nhead=nhead)
    ...
    def forward(self, data_Mgraph, data_TFusion, data_NHGNN):
        ...
        with torch.no_grad():
            y1 = self.model_Mgraph.forward(data_Mgraph, output_vector=False).view(-1,1)
    ...
    def fit(self, train_sets: list, test_sets: list, graph_data: list, device=torch.device("cuda:0"),
        ...
        print(train_sets[0][0].y, train_sets[1][0][2], train_sets[2][0].y)
        print(test_sets[0][0].y, test_sets[1][0][2], test_sets[2][0].y)
        ...
        for epoch in range(epochs):
            ...
            for datas in tqdm(zip(*dataloaders_train), ncols=80):
                ...
                loss = criterion(out.float(), datas[0].y.view(-1, 1).float().cuda())
                loss.backward()
                optimizer.step()
    ...
    def val(self, test_sets: list, graph_data: list, batch_size=128, use_cuda=True, output_pred=False):
