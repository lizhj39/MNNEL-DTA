...
model = TransformerRegressor(save_path=f"saved_models/final_model_{dataset_name}.pt",
                             drug_vocab=drug_vocab, target_vocab=target_vocab, dataset_name=dataset_name,
                             num_layers=2)

model.load_state_dict(torch.load(f"saved_models/final_model_{dataset_name}.pt"))
